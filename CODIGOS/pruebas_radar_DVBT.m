...
pilot_carriers(:) = pilot_carriers - Kmax/2;
pilot_carriers(pilot_carriers<0) = pilot_carriers(pilot_carriers<0) + T_symb;
...
for i=1:((length(s_rx)/(T_symb + CP))+memory_init_offset)
    if (i< avg_symb_num)
        ...
        Y = zeros(1,max(scatter_carriers_aux(subframe_number,:)+1));
        H = zeros(1,max(scatter_carriers_aux(subframe_number,:)+1));
        ...
        H_avg(:,1) = interp1((carriers+1),H(carriers+1),1:T_symb,'linear','extrap')';
        ...
        Ydec(pilot_carriers+1) = ((4/3)*2*(0.5-pilot_inf)); 
